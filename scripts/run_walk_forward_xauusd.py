"""Walk-forward validation for XAUUSD with converged params fixed.

Fixes 16 params that converged (spread < 30%) across the 200-trial
30-param XAUUSD analysis (2026-04-06). Optimizes 14 divergent params.
Bayesian warm-start: best params from window N seed window N+1.

Usage:
    uv run python -m scripts.run_walk_forward_xauusd \\
...
if __name__ == "__main__":
    run_walk_forward_cli(
        ...
        description="Walk-forward validation (XAUUSD, 14 params)",
        default_trials=200,
    )
